...
module Ex201_PSC_Textured
    ...
    function main(;
        ...
        if Plotter !== nothing && !(nameof(Plotter) in [:PyPlot, :PythonPlot])
            @warn "Plotting in Ex201_PSC_Textured is only possible for Plotter = PyPlot or Plotter = PythonPlot"
            Plotter = nothing
        end
        ...
        if Plotter !== nothing
            ...
            Plotter.tripcolor(tridata(subg)..., vcat(nn...), norm = matplotlib.colors.LogNorm(vmin = vmin, vmax = vmax), shading = "gouraud", rasterized = true)
            ...
            #################
            Plotter.figure(figsize = figsize)
            Plotter.tripcolor(tridata(subg)..., vcat(np...), norm = matplotlib.colors.LogNorm(vmin = vmin, vmax = vmax), shading = "gouraud", rasterized = true)
            Plotter.xlabel(" \$x\$ [nm]", fontsize = 17)
            Plotter.ylabel(" \$y\$ [nm]", fontsize = 17)
            Plotter.axis([-20, 770, 20, 800])
            ...
            #################
            Plotter.figure(figsize = figsize)
            Plotter.tripcolor(tridata(subg)..., vcat(na...), norm = matplotlib.colors.LogNorm(vmin = vminIon, vmax = vmaxIon), shading = "gouraud", rasterized = true)
            Plotter.xlabel(" \$x\$ [nm]", fontsize = 17)
            Plotter.ylabel(" \$y\$ [nm]", fontsize = 17)
            Plotter.axis([-20, 770, 20, 800])
