...
immobile acceptor traps in a Gaussian distribution which are coupled to the conduction band.
=#

module Ex111_MoS2_GaussianTraps
...
function main(;Plotter = nothing, plotting = false, verbose = false, test = false, barrierLowering = true, shat=2.0)
    ...
    # Assign the statistics functions
    # For a Gaussian distribution of traps call ``GaussFermiPaasch(shat)'' where shat is the Gaussian width in units of k_B*T.
    # The statistics function is implemented using the Paasch-Scheinert approximation of the Gauss-Fermi integral.
    # The statistics function is also automatically set by the enable_trap_carrier! function, where it is either Gauss-Fermi
    #(if ctsys.data.trapDistributionWidth is positive) or FermiDiracMinusOne (if ctsys.data.trapDistributionWidth is zero).
    data.F = [FermiDiracOneHalfTeSCA, FermiDiracOneHalfTeSCA, GaussFermiPaasch(shat), FermiDiracMinusOne] 
    # 
    # Set trap type for recombination. This is currently also updated automatically by enable_trap_carrier! 
    # because there is only one trap recombination process for shat=0 or shat>0, but this may change in future.
    width_of_trap = shat*k_B*T 
    data.bulkRecombination = set_bulk_recombination(;
        iphin = iphin, iphip = iphip,
        bulk_recomb_Auger = false,
        bulk_recomb_radiative = false,
        bulk_recomb_SRH = false,
        bulk_recomb_trap = ChargeTransport.GaussianDistributedTrap   
    )
    if barrierLowering
    ...
    params.trapDistributionWidth[iphit] = width_of_trap
    ...
    # return maximum(abs.(IV))

    # return biasValues, IV, ctsys
...
function test()
    return main(; verbose = "", test = true, shat=2) ≈ -7435.834149065226 #-6894.342164365617
end

function test2()
    return main(2; verbose = "", test = true) ≈ -7435.834149065226
end

function test4()
    return main(4; verbose = "", test = true) ≈ -7566.037793731334
end

function test6()
    return main(6; verbose = "", test = true) ≈ -7608.847433568394
end
function test8()
    return main(8; verbose = "", test = true) ≈ -7526.183078051347
end
function test10()
    return main(10; verbose = "", test = true) ≈ -7399.327268442193
end
function test30()
    return main(30; verbose = "", test = true) ≈ -6485.896993584058
end
