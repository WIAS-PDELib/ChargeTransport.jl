...
begin
	using Revise
	@warn "Developer Mode - remove following line"
...
The model and discretization is based on xxx. To introduce we shortly recab the stationary 1D van Roosbroeck system, a system to model the charge transport within 1D devices.

The stationary 1D van Roosbroeck system consists of three nonlinear ordinary differential equations for the unknown electrostatic potential $\psi(x)$, the quasi-Fermi potential for electrons $\varphi_n(x)$ and the quasi-Fermi potential for holes $\varphi_p(x)$. It links Poisson’s equation for the electric field to the continuity equations for the carrier densities as follows:
```math
\begin{align}
...
#=
In general more choices are possible. For *three-dimensional bulk semiconductors* the Fermi–Dirac integral of order $\frac{1}{2}$
```math
\mathcal{F}(\eta) := \mathcal{F}_{1/2}(\eta) = \frac{2}{\sqrt{\pi}} \int_{0}^{\infty} 
\frac{\xi^{1/2}}{\exp(\xi - \eta) + 1} \, d\xi
```

and corresponding Blakemore or Bolzmann approximation
```math
...
For *organic semiconductors* the Gauss–Fermi integral
```math
...
\end{align}
```
Signs are handeled internally by ChargeTransport.jl and do not need to be taken into account when entering data.
...
# ╔═╡ e18b8e5f-1582-438d-a242-fab993330b27


# ╔═╡ 5fab15af-ea68-4456-8c8e-1db64e250176
md"""
👉 If you have problems using `PythonPlot.jl`, add `CairoMakie.jl` and change the    	plotter to `Plotter = CairoMakie` in the next cell.
"""
...
# ╔═╡ d93d09b4-2ed1-4d61-9604-dfafaa7794e6
md"""
👉 **Time to get active!** Let us create and plot the grid together.
"""
...
The `System` implments a `VoronoiFVM` system. This is passed to the underlying solver.

We initialize our system with previously defined `data` which is likewise dependent on all parameters.
"""

# ╔═╡ fb6cd5d4-284c-4862-a424-8755d489ba3d
md"""
👉 **Time to get active!** Let us set up the charge transport system together.
"""
...
# ╔═╡ 9ee05fe4-eb1c-43c4-a5cc-1b656725a067
md"""
👉 **Time to get active!** Let us solve the system in thermodynamic equilibrium together.
"""

# ╔═╡ 2c823baf-afd5-4512-ae24-86ce55694778
md"""
👉 **Time to get active!** Let us solve the system for different biases together.
"""
...
# ╔═╡ cc555adb-57c6-4f27-bf85-324d262cc3cb
md"""
👉 **Time to get active!** Let us play around with the parameters.
"""
